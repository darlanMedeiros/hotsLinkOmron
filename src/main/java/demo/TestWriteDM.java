...
/**
 * Writes values 1..1000 to DM 0..1000.
 */
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int addr = 0; addr <= 1000; addr++) {
                int[] value = new int[] { addr + 1 };
                AreaWriteDM write = new AreaWriteDM(plc, addr, value, MemoryWrite.HEX);
                comHandler.send(write);
                System.out.println("WRITE DM addr=" + addr + " status: " + write.getResponseStatusCode());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
